...
async def createUserOrders(
    ...
    stockistExists = await stockist_repo.findOne({
            "_id": order.stockist_id})
    if stockistExists is None:
        raise http_exception.ResourceNotFoundException()
    
    chemist = await chemist_repo.findOne({
...
async def getUserOrders(
    ...
    pipeline = [
        ...
        {
            "$set":{
                "Stockist":{"$arrayElemAt":["$Stockist",0]}
            }
        },
        {
            "$project":{
...
@OrdersRouter.post(
    "/create/orders/details",response_class=ORJSONResponse,status_code=status.HTTP_200_OK
) 
...
@OrdersRouter.get(
    "/get/orders/details/{order_id}",response_class=ORJSONResponse,status_code=status.HTTP_200_OK
) 
async def getOrdersDetails(
    ...
    orderExists = await orders_repo.findOne({
        "_id":order_id
    })
    ...
    response = await order_details_repo.collection.aggregate(pipeline=pipeline).to_list(None)
    return {
        "stockist":{
            "name":data["name"],
...
async def updateOrder(
    ...
    print(updatedDict)       
    await orders_repo.update_one({
        "_id":order_id},{"$set":updatedDict})
...
async def updateOrderDetails(
    ...
    orderDetailsExists = await order_details_repo.findOne({
            "order_id":order_id
        }
    )
    ...
    await order_details_repo.update_one({"order_id":order_id},{"$set":{"product_details":data}})
    return {
        "message":"Order Details updated successfully",
        "success":True
    }
